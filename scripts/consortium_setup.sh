...
# Add contacts and login failure catch
...
curl -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -X POST "$TARGET/graphql" -d '{ "query": "mutation { createConsortium(input: { name: \"MOBIUS\", groupName: \"MOBIUS_CONSORTIUM\", displayName: \"MOBIUS_CONSORTIUM\", dateOfLaunch: \"2024-05-22\", functionalSettings: [ { name: \"Pickup anywhere\", enabled: true, description: \"Pickup anywhere policy\" } ], reason: \"Libraries setup script\", changeCategory: \"Initial setup\" }) { id, name, libraryGroup { id, name }, contacts { id, firstName, lastName }, functionalSettings { id, name } } }" }'

# contacts: [ { firstName: \"Jane\", lastName: \"Doe\", role: \"Consortium admin\", isPrimaryContact: true, email: \"jane.doe@mobius.com\" } ],
