...
curl -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -X POST "$TARGET/graphql" -d '{ "query": "mutation { createConsortium(input: { name: \"MOBIUS\", groupName: \"MOBIUS_CONSORTIUM\", displayName: \"MOBIUS\", reason:\"Consortium creation\", changeCategory: \"Initial setup\",  dateOfLaunch: \"2024-05-22\", contacts: [ { firstName: \"Jane\", lastName: \"Doe\", role: \"Consortium admin\", isPrimaryContact: true, email: \"jane.doe@mobius.com\" } ],functionalSettings: [ { name: PICKUP_ANYWHERE, enabled: false, description: \"Pickup anywhere policy\" }, { name: RE_RESOLUTION, enabled: false, description: \"re-resolution\" }]}) { id, name, libraryGroup { id, name }, contacts { id, firstName, lastName }, functionalSettings { id, name } } }" }'
