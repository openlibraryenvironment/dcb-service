...
```bash
./gradlew :dcb:dockerBuild
```

Around 30 seconds incrementally. There is **no Dockerfile in this repository** and one
should not be added: the image is generated by the micronaut-application plugin from
the `dockerfile { }` block in `dcb/build.gradle`, so `dockerBuild` and the `dockerPush`
that `.github/workflows/release.yml` runs on every push to `main` produce the same
definition. A hand-written Dockerfile alongside it is a second image for one service,
and the JVM flags in that block are exactly the thing that silently drifts.

It tags the local image from the conventional-git version — `8.72.0-SNAPSHOT`, `8-next`,
`8.72-next` and `next`. `next` rather than `latest` because a main snapshot is a
pre-release; a tagged release gets `latest` instead. Without `DOCKER_IMAGE` in the
environment the repository defaults to `docker.libsdev.k-int.com/dcb`, which is the
Nexus `libsdev-docker` repository CI pushes to, so the local tag matches the published
one. Set `DOCKER_IMAGE` to override it.

Two things to know before running the image:

- **It is Alpine** (`BASE_IMAGE` defaults to `eclipse-temurin:25-jre-alpine`), so it has
  no `bash` and no `curl`. A container healthcheck has BusyBox `wget` and nothing else —
  `wget -qO- http://127.0.0.1:8080/health`. A `bash`-based check can only fail, and a
  healthcheck that cannot pass strands everything gated on `service_healthy`.
- **It runs as root.** That is a property of the published artifact, not of local use.

`./gradlew run` remains the faster loop for ordinary development; build the image when
you need to test the thing that actually ships, or to run this service inside a compose
stack.
