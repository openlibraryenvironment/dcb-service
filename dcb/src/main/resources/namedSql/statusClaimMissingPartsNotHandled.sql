...
where pra.audit_data->>'Error' = 'org.olf.dcb.core.interaction.polaris.exceptions.UnhandledItemStatusException: Local item status Claim Missing Parts is unhandled.' and
	  not exists (select 1
				  from patron_request_audit pra1
				  where pra1.patron_request_id = pra.patron_request_id and
						pra1.audit_date > pra.audit_date and
						pra1.audit_data->>'Error' = 'org.olf.dcb.core.interaction.polaris.exceptions.UnhandledItemStatusException: Local item status Claim Missing Parts is unhandled.') and
