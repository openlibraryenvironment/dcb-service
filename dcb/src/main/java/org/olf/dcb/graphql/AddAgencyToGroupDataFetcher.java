...
public class AddAgencyToGroupDataFetcher implements DataFetcher<CompletableFuture<AgencyGroupMember>> {
	...
	public AddAgencyToGroupDataFetcher(AgencyGroupMemberRepository agencyGroupMemberRepository,
			AgencyRepository agencyRepository, AgencyGroupRepository agencyGroupRepository, R2dbcOperations r2dbcOperations) {
	...
	public CompletableFuture<AgencyGroupMember> get(DataFetchingEnvironment env) {
		...
		// input consists of a "group" and "agency" property - each of type ID which
		...
		return Mono.from(r2dbcOperations.withTransaction(status -> {
			...
			return Mono.just(agm).flatMap(agm2 -> group_mono.map(group -> agm2.setGroup(group)))
					.flatMap(agm2 -> agency_mono.map(agency -> agm2.setAgency(agency))).map(agm2 -> {
						UUID record_uuid = UUIDUtils.nameUUIDFromNamespaceAndString(NAMESPACE_DCB,
								"AGM:" + agency_uuid_as_string + ":" + group_uuid_as_string);
