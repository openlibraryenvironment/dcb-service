...
public class ReturnTransitResponse implements LmsResponseStrategy {
	...
	public Map<String, Map<String, Function<String, Map<String, Object>>>> buildResponses() {
		...
		Map<String, Object> loanedResponse = Map.of(
			"itemStatus", ITEM_LOANED,
			"rawItemStatus", ITEM_LOANED,
			"requestStatus", HOLD_MISSING,
			"rawRequestStatus", HOLD_MISSING
		);

		Map<String, Object> availableResponse = Map.of(
			"itemStatus", ITEM_AVAILABLE,
			"rawItemStatus", ITEM_AVAILABLE,
			"requestStatus", HOLD_MISSING,
			"rawRequestStatus", HOLD_MISSING
		);
		...
		stdResponse.put(SUPPLIER_ROLE, state -> loanedResponse);
		stdResponse.put(BORROWER_ROLE, state -> availableResponse);

		Map<String, Function<String, Map<String, Object>>> puaResponse = new HashMap<>();
		puaResponse.put(SUPPLIER_ROLE, state -> loanedResponse);
		puaResponse.put(BORROWER_ROLE, state -> loanedResponse);
		puaResponse.put(PICKUP_ROLE, state -> availableResponse);
	...
	public boolean supports(String state, String role, String responseType) {
		return "RETURN_TRANSIT".equalsIgnoreCase(state);
	}
