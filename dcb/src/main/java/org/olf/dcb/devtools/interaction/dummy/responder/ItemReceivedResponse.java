...
public class ItemReceivedResponse implements LmsResponseStrategy {
	...
	public Map<String, Map<String, Function<String, Map<String, Object>>>> buildResponses() {
		...
		Map<String, Object> transitResponse = Map.of(
			"itemStatus", ITEM_TRANSIT,
			"rawItemStatus", ITEM_TRANSIT,
		...
		stdResponse.put(SUPPLIER_ROLE, state -> transitResponse);
		stdResponse.put(BORROWER_ROLE, state -> receivedResponse);

		Map<String, Function<String, Map<String, Object>>> puaResponse = new HashMap<>();
		puaResponse.put(SUPPLIER_ROLE, state -> transitResponse);
		puaResponse.put(BORROWER_ROLE, state -> transitResponse);
		puaResponse.put(PICKUP_ROLE, state -> receivedResponse);
	...
	public boolean supports(String state, String role, String responseType) {
		return "RECEIVED_AT_PICKUP".equalsIgnoreCase(state);
	}
