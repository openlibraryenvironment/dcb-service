...
public class LoanedResponse implements LmsResponseStrategy {
	...
	public Map<String, Map<String, Function<String, Map<String, Object>>>> buildResponses() {
		...
		stdResponse.put(SUPPLIER_ROLE, state -> onHoldShelfResponse);
		stdResponse.put(BORROWER_ROLE, state -> loanedResponse);

		Map<String, Function<String, Map<String, Object>>> puaResponse = new HashMap<>();
		puaResponse.put(SUPPLIER_ROLE, state -> onHoldShelfResponse);
		puaResponse.put(BORROWER_ROLE, state -> onHoldShelfResponse);
	...
	public boolean supports(String state, String role, String responseType) {
		return "LOANED".equalsIgnoreCase(state);
	}
