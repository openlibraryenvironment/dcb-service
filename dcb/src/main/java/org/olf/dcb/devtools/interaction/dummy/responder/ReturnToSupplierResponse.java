...
public class ReturnToSupplierResponse implements LmsResponseStrategy {
	...
	public Map<String, Map<String, Function<String, Map<String, Object>>>> buildResponses() {
		...
		stdResponse.put(SUPPLIER_ROLE, state -> availableResponse);
		stdResponse.put(BORROWER_ROLE, state -> loanedResponse);

		Map<String, Function<String, Map<String, Object>>> puaResponse = new HashMap<>();
		puaResponse.put(SUPPLIER_ROLE, state -> availableResponse);
		puaResponse.put(BORROWER_ROLE, state -> loanedResponse);
		puaResponse.put(PICKUP_ROLE, state -> loanedResponse);
