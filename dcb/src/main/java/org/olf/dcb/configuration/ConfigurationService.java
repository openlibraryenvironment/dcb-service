...
/**
 * The resource sharing network is composed of Agencies hosted on HostLMS
 * systems (A single HostLMS can play host to many Agencies). This service polls
 * the different HostLMS systems and attempts to extract configuraiton data
...
public class ConfigurationService implements Runnable {
	...
	public ConfigurationService(List<IngestSourcesProvider> sourceProviders,
			ShelvingLocationRepository shelvingLocationRepository, AgencyRepository agencyRepository,
			LocationRepository locationRepository, RefdataValueRepository refdataValueRepository) {
	...
	public Flux<ConfigurationRecord> getConfigRecordStream() {
		return Flux
				.merge(Flux.fromIterable(sourceProviders).concatMap(IngestSourcesProvider::getIngestSources).filter(source -> {
					if (source.isEnabled())
						return true;
					log.info("Ingest from source: {} has been disabled in config", source.getName());
					return false;
				}).map(IngestSource::getConfigStream).onErrorResume(t -> {
...
//		return new ShelvingLocation().builder().id(lr.getId()).code(lr.getCode()).name(lr.getName())
//				.hostSystem((DataHostLms) br.getLms()).location(l).build();
//	}
	...
	private Location mapLocationRecordToSubLocation(LocationRecord lr, Location l, BranchRecord br) {
		...
		return Location.builder().id(lr.getId()).code(lr.getCode()).name(lr.getName())
				.hostSystem((DataHostLms) br.getLms()).parentLocation(l).type("LOCATION").build();
	}

	@Transactional(value = TxType.REQUIRED)
	public Mono<ShelvingLocation> upsertShelvingLocation(ShelvingLocation sl) {
		log.debug("upsertShelvingLocation {}", sl);
		return Mono.from(shelvingLocationRepository.existsById(sl.getId())).flatMap(
				exists -> Mono.fromDirect(exists ? shelvingLocationRepository.update(sl) : shelvingLocationRepository.save(sl)))
		// .map(exists -> Mono.fromDirect(exists ? shelvingLocationRepository.update(sl)
		// : shelvingLocationRepository.save(sl)))
		// .thenReturn(sl)
		;
	}
	...
	public Mono<Location> upsertLocation(Location l) {
		...
		return Mono.from(locationRepository.existsById(l.getId()))
				...
				// .map(exists -> Mono.fromDirect(exists ? locationRepository.update(l) :
	...
	public Mono<Void> createSubLocations(Location l, BranchRecord br) {
		...
		return Flux.fromIterable(br.getSubLocations()).map(slr -> mapLocationRecordToSubLocation(slr, l, br))
				.doOnNext(locrec -> log.debug("Attempt to upsert sub location {}", locrec))
				.concatMap(locrec -> upsertLocation(locrec)).then(Mono.empty());
	}

	@Transactional(value = TxType.REQUIRED)
	public Mono<Agency> handleAgencyRecord(BranchRecord br) {
		// Different host LMS systems will have different policies on how BranchRecords
		// map to agencies
		// In a multi-tenant sierra for example, branch records represent institutions
		// and we should create
		// an agency for each branch record. This method will take account of policies
		// configured for the
		...
		if (br.getLms() instanceof DataHostLms) {
			DataAgency upsert_agency = DataAgency.builder().id(br.getId())
					.code(br.getLms().getCode() + "-BR-" + br.getLocalBranchId()).name(br.getBranchName())
					.hostLms((DataHostLms) br.getLms()).build();

			// log.debug("upsertAgency {}", br);
			return Mono.from(agencyRepository.existsById(upsert_agency.getId())).flatMap(exists -> Mono
					.fromDirect(exists ? agencyRepository.update(upsert_agency) : agencyRepository.save(upsert_agency)));
		} else {
	...
	/**
	 * Given a BranchRecord which includes a UUID5 that globally and consistently
	 * identifies the branch (In the context of a HostLMS) Assuming that we wish to
	 * create an Agency record that corresponds to the branch, create the Agency and
	 * then check to see if we need to create any ShelvingLocation records
	 *
	 * @param br
	 * @return
	 */
	...
	public Mono<Location> handleBranchRecord(BranchRecord br) {
		...
		if ((br.getLms() instanceof DataHostLms) && (br.getLocalBranchId() != null)) {
			Location l = Location.builder().id(br.getId()).code(br.getLocalBranchId())
					.name(br.getBranchName() != null ? br.getBranchName() : br.getLocalBranchId())
					.hostSystem((DataHostLms) br.getLms()).type("BRANCH").build();

			// return Mono.from(locationRepository.existsById(l.getId()))
			// .doOnNext(exists -> log.debug("Check if location with id {} exists
			// ?",l.getId(),exists))
			// .flatMap(exists -> Mono.fromDirect(exists ? locationRepository.update(l) :
			// locationRepository.save(l)))

			return upsertLocation(l).flatMap(savedLocation -> createSubLocations(savedLocation, br)).thenReturn(l);
	...
	protected Mono<Location> handlePickupLocation(PickupLocationRecord pickupLocationRecord) {
		...
		if (pickupLocationRecord.getLms() instanceof DataHostLms) {
			Location upsert_location = Location.builder().id(pickupLocationRecord.getId())
					.code(pickupLocationRecord.getCode()).name(pickupLocationRecord.getName())
					.hostSystem((DataHostLms) pickupLocationRecord.getLms()).isPickup(true).type("PICKUP").build();

			return upsertLocation(upsert_location).thenReturn(upsert_location);
		} else {
			log.warn("Unable to save agency for statically configured HostLMS");
			return Mono.empty();
		}
	...
	/**
	 * We don't know what kinds of configuration records a hostLms might emit, so
	 * here we handle all the different possible cases. At the moment BranchRecords
	 * and PickupLocations.
	 *
	 * @param cr - A canonical branch record which tries to minimise the differences
	...
	public Mono<ConfigurationRecord> handleConfigRecord(ConfigurationRecord cr) {
		...
		return Mono.just(cr)
				// We only handle branch records at the moment - and the HostLMS must be a data
				// host lms
				.flatMap(confrec -> {
	...
	protected Mono<RefdataValue> handleRefdataRecord(RefdataRecord refdataRecord) {
		...
		RefdataValue rdv = RefdataValue.builder().id(refdataRecord.getId()).category(refdataRecord.getCategory())
				.context(refdataRecord.getContext()).label(refdataRecord.getLabel()).value(refdataRecord.getValue()).build();

		return Mono.from(refdataValueRepository.existsById(rdv.getId())).flatMap(
				exists -> Mono.fromDirect(exists ? refdataValueRepository.update(rdv) : refdataValueRepository.save(rdv)));
	}
