...
//				state.current_page = bibs;
	...
	public Mono<List<Item>> getItemsByBibId(String bibId, String hostLmsCode) {
		...
		return Mono
				.from(client.items(params -> params.deleted(false).bibIds(List.of(bibId))
	...
	public Mono<HostLmsItem> getItem(String itemId) {
		...
		return Mono.from(client.getItem(itemId)).flatMap(sierraItem -> Mono.just(sierraItemToHostLmsItem(sierraItem)))
