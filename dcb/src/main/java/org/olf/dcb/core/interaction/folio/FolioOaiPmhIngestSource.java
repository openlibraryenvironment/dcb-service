	  		...
	  		.flatMap( lrr -> {
	  			...
	  			return Mono.just(lrr)
			  		...
			  		.flatMap(jsonArr -> {
			  			...
			  			// If the array is empty then we should use the "from"
			  			ListRecordsParamsBuilder lrp;
			  			if (jsonArr.size() == 0 || !StringUtils.hasText(lrr.resumptionToken())) {
			  				lrp = ListRecordsParams.builder()
			  					.resumptionToken(null)
			  					.from(now);
			  			} else {
			  				lrp = ListRecordsParams.builder()
			  					.resumptionToken(lrr.resumptionToken());
			  			}
			  			
			  			return reactiveObjectMap(lrp.build())
			  				.map(Optional::of)
			  				.defaultIfEmpty(Optional.empty())
			  				.map( nextToken -> Tuples.of(jsonArr, nextToken) );
			  		})
						.flatMap(TupleUtils.function( (jsonArr, nextToken) -> {
							
							final var builder = SourceRecordImportChunk.builder()
									.lastChunk( jsonArr.size() < 1 || nextToken.isEmpty() )
									.checkpoint( nextToken.orElseGet( null ) );
