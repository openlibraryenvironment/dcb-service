...
public class IndexSynch implements Job<UUID>, JobChunkProcessor {
  ...
  protected static class JobParameters {
	...
	public <T> Publisher<JobChunk<T>> processChunk(JobChunk<T> chunk) {
		...
		return Flux.just( indexChunk )
			.map( IndexJobChunk::getData )
			.map( data -> (List<UUID>)new ArrayList<>(data) )
		...
		return Mono.justOrEmpty(chunk)
			...
			.map( cutoff -> {
				...
				Mono.just(cutoff)
					...
					.doOnError( _t -> log.warn("Error attempting to delete documents from Index.") )
					.retry(10)
					.subscribe();
