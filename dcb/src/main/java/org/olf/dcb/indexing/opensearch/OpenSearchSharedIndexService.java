...
//		return(indexName);
	...
	private TypeMapping getTypeMappings(JsonpMapper mapper) {
		...
		final String location = RESOURCE_SHARED_INDEX_MAPPING_PREFIX + String.valueOf(indexVersion) + RESOURCE_SHARED_INDEX_POSTFIX;
		
		log.info("Using mappings file: [{}]", location);
  	// Read the mappings and generate a TyypeMcreateUpdateIndexMappingsapping from them
  	InputStream mappingsInputStream = getClass().getClassLoader().getResourceAsStream( location );
