...
//
//		Mono<Agency> agency_mono = Mono.from(agencyRepo.save(new Agency(UUID.randomUUID(), "ReShareDCB Test Institution A")));
//		log.debug("Created test agency: {}", agency_mono.block().toString());
//
//		// Make sure that we have 1 task
//		List<org.olf.reshare.dcb.core.model.Agency> agencies = Flux.from(agencyRepo.findAll()).collectList().block();
